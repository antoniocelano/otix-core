...
class S3 {
    ...
    // Aggiungi la nuova proprietà per la regione
    private $region = '';
	 ...
	 */
    public function __construct(string $access_key, string $secret_key, string $endpoint = '', string $region = '') {
        $this->access_key = $access_key;
        $this->secret_key = $secret_key;
        $this->endpoint = $endpoint ?: self::DEFAULT_ENDPOINT;
        $this->region = $region; // Inizializza la regione
     ...
     * @return S3Response
     */
    public function getBucket(string $bucket, array $headers = [], array $params = []): S3Response {
        $request = (new S3Request('GET', $this->endpoint, $bucket, $params))
            ->setHeaders($headers)
            ->useMultiCurl($this->multi_curl)
            ->useCurlOpts($this->curl_opts)
            // Passa la regione alla richiesta prima di firmare
