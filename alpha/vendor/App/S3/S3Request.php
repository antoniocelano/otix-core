...
class S3Request {
    ...
    // Aggiungi questa nuova proprietà
    private $region = '';
	 ...
	 */
    public function __construct(string $action, string $endpoint, string $uri, array $params = []) {
        $this->action = $action;
	...
	/**
     * Imposta la regione manualmente.
    ...
    public function sign(string $access_key, string $secret_key, string $endpoint) {
        ...
        $service = 's3';
        // Usa la regione impostata se non è vuota
        $region = $this->region ?: $this->getRegion($endpoint);
        ...
        $canonical_query = http_build_query($this->params, '', '&', PHP_QUERY_RFC3986);
